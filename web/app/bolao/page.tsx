        ...
        {cicloAtivo && (
          <div className="bg-surface rounded-lg p-6 space-y-5">
            <div className="flex items-start justify-between">
              <div>
                <h2 className="text-brand font-semibold text-lg">
                  Concurso #{cicloAtivo.concurso_nr}
                </h2>
                <span className="text-muted text-xs">
                  {cicloStatusLabel[cicloAtivo.status as CicloStatus]}
                </span>
              </div>
                      ...
                      {p.status === 'aprovado' && (
                        <div className="text-right">
                          <p className="text-muted text-xs">
                            {fmt(p.valor_pago)} / {fmt(valorDevido)}
                          </p>
                        ...
                        <div className="flex gap-2">
                          ...
                          {p.status === 'aprovado' && !pago && (
                            <form action={marcarPagamentoAction.bind(null, p.id, valorDevido)}>
                              <button type="submit" className="text-xs bg-primary border border-muted/30 text-muted hover:text-brand px-2 py-1 rounded cursor-pointer">
                                Marcar pago
                              </button>
                            </form>
                          )}
...
async function marcarPagamentoAction(participacaoId: number, valor: number, _formData: FormData) {
  'use server'
